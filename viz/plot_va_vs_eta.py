...
hicieron a un amigo + capturas de sus gráficos), con el mismo criterio de "todo en un panel"
tanto con uno como con los dos modelos cargados:
  - Con UN solo modelo cargado (--models=VICSEK, típico de comparaciones contra otro grupo
    ...
    coloreadas por rho.
  - Con AMBOS modelos cargados (default, para el informe / punto f): TAMBIEN todas las
    densidades van en un mismo panel -- coloreadas por rho, distinguiendo el modelo por trazo
    (solido Vicsek / punteado Votante, via MODEL_STYLE) en vez de separar en subplots por rho
    (se dejo de usar subplots aca por pedido explicito: separar en subplots para el punto f
    quedaba como "repetir" el panel unico de cada densidad tres veces en vez de una comparacion
    conjunta).
"""
    ...
    for rho in all_densities:
        ...
        for model in selected_models:
            ...
            ax.errorbar([row["eta"] for row in selected], [row["mean_va"] for row in selected],
                        ...
                        linestyle=linestyle, capsize=3, linewidth=1.3, markersize=5, label=label)
